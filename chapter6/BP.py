...
class BPNet(nn.Module):
    ...
    def predict(self,x,y):
        ...
        with torch.no_grad():
            y_hat = self.forward(x)
            l = loss(y_hat,y)
            pred = torch.argmax(y_hat, dim=1)  # 或 softmax 后取最大概率
            # 处理 y 可能为 one-hot 或者直接的 label 索引
            ...
            for p, t in zip(pred_cpu, true_cpu):
                if int(p.item()) == int(t.item()):
                    equal_count += 1
            print(f"BP acc:{equal_count/pred_cpu.size()[0]}")
            # 返回相等的个数
            return l.item()
...
if __name__ == '__main__':#西瓜数据集上的测试
    ...
    loss = net1.predict(train_features,train_labels)
    #动态调整学习率可能在比较复杂的情况下比较好用，学习率调得很高都能收敛的情况下可能不用降低学习率
